...
See https://github.com/schauder/Dependency-Manager for details""")
        ...
        val excludeFilter = opt[List[String]]("excludeFilter",
            default = Some(List()),
            descr = """if this argument is given those nodes that are matched by 
                the given regular expression will get excluded from the graph.""")
        val includeFilter = opt[List[String]]("includeFilter",
            default = Some(List()),
            descr = """if this argument is given, 
                only those nodes get included in the resulting graph,
